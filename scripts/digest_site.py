...
<body><main id="top"><header><h1>每日 AI 日报 · {edition_label}</h1><p>{now:%Y-%m-%d %H:%M}（北京时间）· 最近三个自然日 · {len(usable)} 条有效资讯</p><p><a href="{html.escape(other_file, quote=True)}">切换到{'下午篇' if edition_slug == 'morning' else '上午篇'}</a></p></header>
